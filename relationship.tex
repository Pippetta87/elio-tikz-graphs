...
\begin{tikzpicture}[node distance=1.5cm, every edge/.style={link}]
  ...
  \node[relationship] (calibration) [above =1.5cm of opacity] {Calibrazione luminosit\'a e raggio} edge[<-] (ssm);

\node[entity] (unphys) [above right=1cm and 2cm of calibration] {Incertezze fisica del modello} edge [<-] (ssm);

\node[entity] (corrections) [below right=2.5cm and 1.5 of ssm] {Correzioni} edge [->] (ssm);
  \node[entity] (modelobs) [right=2.5cm and 1.5cm of ssm] {Discrepanze modello/osservazioni} edge [->] (corrections);
  \node[entity] (surfaceobs) [above right=1cm and 2cm of ssm] {Osservabili stellari} edge[->] (modelobs);
  \node[entity,anchor=north west] (resismobs) at (surfaceobs.south east) {Osservabili sismologiche};
  \draw[<-]  (sismobs.20)--++(30:2) node[attribute] (freqs) {Frequenze oscillazioni adiabatiche} edge (sismobs);
  \draw[<-] (sismobs.5)--++(5:2) node[attribute] (radiuscz) {Raggio fondo zona convettiva} edge (sismobs);
