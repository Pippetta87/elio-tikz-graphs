...
\begin{tikzpicture}[node distance=1.5cm, every edge/.style={link}]
  ...
  \node[isa] (calibration) [below=2.5cm of ssm] {\parbox{4cm}{Parametri fisici da determinare tramite confronto con osservazioni: efficenza trasporto convettivo, abbondanza di elio iniziale}} edge (ssm);

  \node[entity] (stelparam) [below=1cm of ssm] {Parametri stellari} edge (calibration);
  \node[entity] (sismobs) [right=1cm of ssm] {Osservabili sismologiche} edge (ssm);

  \node[relationship] (modobs) [left=2cm of ssm] {Discrepanze modello/osservazioni} edge (ssm);
...
\begin{comment}
  ...
  \node[relationship] (buy) [below=of stelparam] {Buys};
  ...
  \draw[link] (car.10) -| (buy) (buy) edge (stelparam);
  \draw[link] (car.-10) -| (sel) (sel) |- (stelparam);
