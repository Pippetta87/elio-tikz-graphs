...
\tikzstyle{every attribute} = [fill=yellow!20!white!100, draw=yellow, node distance=1cm, drop shadow,text width=1.5cm, fill opacity=0.5]
...
\draw[link,->] (sismobs.320)--++(320:2cm) node[attribute] (heliumenv) {$Y_{ph}$};
\draw[link,->] (sismobs.340)--++(340:3cm) node[attribute] (soundspeed) {Velocit\'a del suono (isothermal): $c_s(u)$};
\draw[link,->] (sismobs.300)--++(300:2cm) node[attribute] (densitybcz) {Densit\'a fondo zona convettiva $\rho_{cz}$};
