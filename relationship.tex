...
\draw[<-]  (ssm.190)--++(190:2cm) node[attribute] (hydro) {Equilibrio idrostatico};
\draw[<-]  (ssm.170)--++(170:2cm) node[attribute] (thermal) {Equilibrio termico locale};
\draw[<-]  (ssm.175)--++(175:2cm) node[attribute] (opacity) {Equilibrio radiativo + criterio di Schwartzchild};
...
\draw[link,<->] (surfaceobs.300) -- node [pos=0.8, auto, swap,left] (stellaracc) {accuratezza};
\draw[link,<->] (sismobs.180) -- node [pos=0.2, auto, swap,right] (sismacc) {accuratezza};
